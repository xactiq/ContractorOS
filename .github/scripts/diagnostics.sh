...
if [ -z "$SUPA_URL" ]; then
    ...
    # Note: the bare /rest/v1/ root always returns 401 from PostgREST (no resource requested);
    # that's expected behavior, not an auth failure. Query a real table instead.
    ...
    if [[ "$SUPA_STATUS" =~ ^2 ]]; then
        ...
        MAJOR_CHANGES="${MAJOR_CHANGES}\n- **Supabase Auth:** anon key rejected (HTTP $SUPA_STATUS) on real table query — verify key in Supabase dashboard."
    ...
    for TABLE in clients jobs estimates supplements docs; do
        # Content-Range is "start-end/total" when rows exist, or "*/total" (e.g. "*/0") when empty —
        # the old regex only matched the numeric-range form, so empty tables always showed N/A.
