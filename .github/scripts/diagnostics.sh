...
if [ -z "$SUPA_URL" ]; then
    ...
    # NOTE: the bare /rest/v1/ root requires a service_role key on current
    # Supabase gateways and always 401s for an anon key — that is expected,
    # not a health signal. Query a real table instead.
    ...
    if [[ "$SUPA_STATUS" =~ ^2 ]]; then
        ...
        fail "Supabase REST: HTTP $SUPA_STATUS — anon key rejected on table query; verify the API key is active in Supabase dashboard"; append "- **REST API:** ❌ HTTP $SUPA_STATUS (anon key rejected)"
        MAJOR_CHANGES="${MAJOR_CHANGES}\n- **Supabase auth:** anon key rejected on a real table query (HTTP $SUPA_STATUS) — app cannot read/write data."
