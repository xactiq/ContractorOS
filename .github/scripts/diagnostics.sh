...
if [ -z "$SUPA_URL" ]; then
    ...
    if [[ "$SUPA_STATUS" =~ ^2 ]]; then
        ...
        fail "Supabase REST: HTTP 401 — anon key rejected on real table query"; append "- **REST API:** ❌ HTTP 401 (anon key rejected)"
        MAJOR_CHANGES="${MAJOR_CHANGES}\n- **Supabase anon key rejected:** live app cannot read data. Verify the key in the Supabase dashboard."
